...
static const char *ResultCharacterName(i32 index)
{
    static const char *keys[6] = {
        "th07 Stats ReimuA", "th07 Stats ReimuB", "th07 Stats MarisaA",
        "th07 Stats MarisaB", "th07 Stats SakuyaA", "th07 Stats SakuyaB",
    };
    return index >= 0 && index < 6 ? THPatch_StringDef(keys[index], g_CharacterList[index]) : "";
}

static const char *ResultAllCharactersName()
{
    return THPatch_StringDef("th07 Stats Total (All Characters)", g_TotalForAllProtagonists);
}
    ...
    case 8:
        if (arg->charUsed != arg->cursor && arg->frameTimer == 20)
        {
            ...
            g_AnmManager->DrawStringFormat2(arg->spellcardListVms, 0xffffff, 0,
                                            ResultCharacterName(arg->charUsed));
    ...
    case 9:
            ...
            arg->frameTimer == 20)
        {
            ...
            AnmManager::DrawVmTextFmt(
                ...
                THPatch_StringDef("th07 Spell Result Character Select",
                                  "%s %3d枚中%3d枚取得（キャラ切り替え↓↑）"),
                ResultCharacterName(arg->prevSpellcardListPage), 141,
...
ZunResult ResultScreen::HandleResultKeyboard()
{
    ...
    if (this->frameTimer == 0)
    {
        ...
        g_AnmManager->DrawStringFormat2(this->spellcardListVms, 0xffffff, 0,
                                        ResultCharacterName(this->charUsed));
...
ZunResult ResultScreen::HandleReplaySaveKeyboard()
{
    ...
    case 13:
        ...
        {
#ifdef TH_PS4
            // Relative paths resolve inside the read-only /app0 here, and the throwing
            // overload would end the game just as the player saves a replay.
            std::error_code replayDirError;
            std::filesystem::create_directory(TH_PS4_GAME_DIR "/replay", replayDirError);
#else
...
i32 ResultScreen::DrawStats()
{
    ...
    case 20:
        if (this->frameTimer == 1)
        {
            ...
            vm->pos = pos;
            g_Supervisor.UpdateStartupTime();
            AnmManager::DrawVmTextFmt(g_AnmManager, vm, 0xffffff, 0, "%s   %.2d:%.2d:%.2d",
                                      THPatch_StringDef("Stats Time Since Startup", "総起動時間"),
            ...
            vm->pos = pos;
            AnmManager::DrawVmTextFmt(g_AnmManager, vm, 0xffffff, 0, "%s %.2d:%.2d:%.2d",
                                      THPatch_StringDef("Stats Total Playtime", "総プレイ時間"),
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(
                    ...
                    "%s          Easy  Norm  Hard  Luna Extra Phant. Total",
                    THPatch_StringDef("Stats Play Count", "プレイ回数"));
            }
            else
            {
                AnmManager::DrawVmTextFmt(
                    g_AnmManager, vm, 0xffffff, 0,
                    "%s          Easy  Norm  Hard  Luna Extra Total",
                    THPatch_StringDef("Stats Play Count", "プレイ回数"));
            }
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(
                    g_AnmManager, vm, 0xffffff, 0, "%s %6d %6d %6d %6d %6d %6d %6d",
                    ResultAllCharactersName(), g_GameManager.plst.playDataByDifficulty[0].playCount,
            ...
            else
            {
                AnmManager::DrawVmTextFmt(g_AnmManager, vm, 0xffffff, 0,
                                          "%s %6d %6d %6d %6d %6d %6d", ResultAllCharactersName(),
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(
                    g_AnmManager, vm, 0xffffff, 0, "%s          %6d %6d %6d %6d %6d %6d %6d",
                    THPatch_StringDef("Stats Clear Count", "クリア回数"),
                    g_GameManager.plst.playDataByDifficulty[0].noContinueClearCount,
                    g_GameManager.plst.playDataByDifficulty[1].noContinueClearCount,
                    g_GameManager.plst.playDataByDifficulty[2].noContinueClearCount,
                    g_GameManager.plst.playDataByDifficulty[3].noContinueClearCount,
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(g_AnmManager, vm, 0xffffff, 0,
                                          ...
                                          THPatch_StringDef("Stats Continue", "コンティニュー"),
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(
                    g_AnmManager, vm, 0xffffff, 0, "%s          %6d %6d %6d %6d %6d %6d %6d",
                    THPatch_StringDef("Stats Practice", "プラクティス"),
            ...
            if (g_GameManager.HasUnlockedPhantomAndMaxClears())
            {
                AnmManager::DrawVmTextFmt(g_AnmManager, vm, 0xffffff, 0,
                                          "%s          %6d %6d %6d %6d %6d %6d %6d",
                                          THPatch_StringDef("Stats Retries", "リトライ回数"),
