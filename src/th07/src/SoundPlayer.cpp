...
#ifdef TH_PS4
static void PS4_AudioCallback(void *userdata, Uint8 *stream, i32 len)
{
    ...
    // A device-less miniaudio engine mixes as interleaved stereo float samples. SDL's
    // PS4 driver owns AudioOut and invokes us at its own cadence.
    const ma_uint64 frames = (ma_uint64)len / (sizeof(f32) * 2);
    ma_engine_read_pcm_frames(player->engine, stream, frames, nullptr);
}
#endif
...
ZunResult SoundPlayer::InitializeSound()
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0)
    {
        ...
        SAFE_DELETE(this->engine);
        g_GameErrorContext.Log("PS4 audio subsystem initialization failed\n");
        return ZUN_ERROR;
    }
    ...
    desired.callback = PS4_AudioCallback;
    desired.userdata = this;
    this->audioDevice = SDL_OpenAudioDevice(nullptr, 0, &desired, nullptr, 0);
    if (this->audioDevice == 0)
    {
        ma_engine_uninit(this->engine);
        SAFE_DELETE(this->engine);
        g_GameErrorContext.Log("PS4 audio device initialization failed\n");
        return ZUN_ERROR;
    }
