...
#ifdef TH_PS4
// Cross shoots, Circle bombs, R1 focuses, Options pauses and Triangle skips text. The
// indices are into g_DIToSDLButton in Controller.cpp, not raw pad buttons. Directions stay
// unmapped: the D-pad and sticks are read separately.
const ControllerMapping g_DefaultPS4ControllerMapping = {0, 1, 5, 7, -1, -1, -1, -1, 3};
ControllerMapping g_ControllerMapping = g_DefaultPS4ControllerMapping;
#else
...
#ifdef TH_PS4
        // A th07.cfg written before the PS4 layout existed would bring back the PC mapping,
        // and the console's buttons aren't remappable from the menu, so impose it here.
        g_ControllerMapping = g_DefaultPS4ControllerMapping;
        g_Supervisor.cfg.controllerMapping = g_ControllerMapping;
#endif
    }
