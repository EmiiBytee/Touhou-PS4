...
#ifdef TH_THPATCH
// The menu help lines live in the game's own code, not in a texture, so thcrap replaces
// them through its string table instead of through an image. Without this the patch's
// English text and the game's Japanese one end up on screen at the same time.
static const char *const kMenuKeys[8] = {
    "th07 Menu Start",  "th07 Menu Extra Start", "th07 Menu Practice Start", "th07 Menu Replay",
    "th07 Menu Result", "th07 Menu Music Room",  "th07 Menu Option",         "th07 Menu Quit",
};
static const char *const kOptionKeys[9] = {
...
static const char *const kKeyConfigKeys[12] = {
...
static const char *Translated(const char *const *keys, u32 count, const char *const *fallback, u32 i)
{
    return i < count ? THPatch_StringDef(keys[i], fallback[i]) : fallback[i];
}

#define TH_MENU_STRING(i) Translated(kMenuKeys, 8, g_MainMenuStrings, (u32)(i))
#define TH_OPTION_STRING(i) Translated(kOptionKeys, 9, g_OptionsStrings, (u32)(i))
#define TH_KEYCONFIG_STRING(i) Translated(kKeyConfigKeys, 12, g_KeyConfigStrings, (u32)(i))
#else
#define TH_MENU_STRING(i) g_MainMenuStrings[i]
#define TH_OPTION_STRING(i) g_OptionsStrings[i]
#define TH_KEYCONFIG_STRING(i) g_KeyConfigStrings[i]
#endif
...
u32 MainMenu::OnUpdateOptionsMenu()
{
    ...
    case 0:
        ...
        for (i = 0; (u32)i < 9; i++)
        {
            g_AnmManager->DrawStringFormat2(&this->vms[i], 0xfff0e0, 0x300000, TH_OPTION_STRING(i));
        }
    case 1:
        break;
    }
...
skip_right_sound:
    ...
    if (WAS_PRESSED_RAW(TH_BUTTON_SELECTMENU))
    {
        ...
        LAB_00456cc0:
            ...
            g_SoundPlayer.ProcessQueues();
#ifdef TH_PS4
            // The screen-mode option changes the GNM viewport between 4:3 and 16:9 at
            // runtime. Recreating the desktop window/renderer is invalid on PS4.
            if (false)
#else
...
#ifdef TH_PS4
            // The PS4 has no working chdir, and only fopen/mkdir are redirected, so a
            // relative path here would point into the read-only /app0 and the throwing
            // filesystem calls below would kill the game. Name the real directory, and use
            // the error_code overloads: an unreadable replay folder must not be fatal.
            const fs::path replay = TH_PS4_GAME_DIR "/replay";
#else
            const fs::path replay = "./replay";
#endif
            std::error_code replayError;
            fs::create_directory(replay, replayError);

            std::vector<fs::directory_entry> entries;
            for (fs::directory_iterator it(replay, replayError), end; it != end; it.increment(replayError))
            {
                if (replayError)
                {
                    break;
