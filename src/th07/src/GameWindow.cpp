...
void GameWindow::Present()
{
    ...
    if (WAS_PRESSED_RAW(TH_BUTTON_HOME))
    {
        ...
        std::filesystem::create_directory(TH_PS4_GAME_DIR "/snapshot", snapshotDirError);
#else
...
ZunResult GameWindow::CreateGameWindow()
{
    ...
    // Piglet/SDL OpenGL cannot initialize in a PS4 "big app". GNM owns VideoOut directly;
    // SDL is still used for timing, events and controller input.
    const u32 sdlFlags = SDL_INIT_TIMER | SDL_INIT_EVENTS;
#else
    const u32 sdlFlags = SDL_INIT_VIDEO;
#endif
