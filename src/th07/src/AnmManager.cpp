...
ZunResult AnmManager::LoadTexture(i32 textureIdx, const char *texturePath, u32 colorKey)
{
    ...
    ZunResult result = LoadTextureFromMemory(textureIdx, srcData, g_LastFileSize, colorKey);
    free(srcData);
    return result;
}

ZunResult AnmManager::LoadTextureFromMemory(i32 textureIdx, const u8 *data, u32 size,
                                            u32 colorKey)
{
...
#ifdef TH_THPATCH
// thcrap patches ANM sheets sprite by sprite. A completely transparent sprite means "keep the
// one from the previous patch (or the game)", while any non-transparent pixel means "replace
// this whole sprite". Alpha-blending the whole sheet keeps the untranslated Japanese pixels;
// replacing the whole sheet drops portraits and HUD glyphs that the patch deliberately leaves
// transparent. Reproduce thcrap's sprite-level rule here.
static void ApplyThpatchSprites(SDL_Surface *base, const char *name, const AnmRawEntry *entry)
{
    if (!base || !name || !entry)
    {
        return;
    ...
    u32 size = 0;
    u8 *patch = THPatch_LoadReplacement(name, &size);
    if (!patch)
    {
        return;
    }
    SDL_RWops *rw = SDL_RWFromConstMem(patch, size);
    SDL_Surface *loaded = rw ? IMG_Load_RW(rw, 1) : NULL;
    SDL_Surface *overlay = loaded ? SDL_ConvertSurfaceFormat(loaded, SDL_PIXELFORMAT_RGBA32, 0)
                                  : NULL;
    if (loaded)
    {
    ...
    if (overlay)
    {
        ...
        const f32 scaleX = entry->width ? (f32)base->w / (f32)entry->width : 1.0f;
        const f32 scaleY = entry->height ? (f32)base->h / (f32)entry->height : 1.0f;
        const i32 *spriteOffset = entry->dataOffsets;
        for (i32 i = 0; i < entry->numSprites; i++, spriteOffset++)
        {
            const AnmRawSprite *sprite =
                (const AnmRawSprite *)((const u8 *)entry + *spriteOffset);
            SDL_Rect rect = {
                (i32)std::floor(sprite->offset.x * scaleX),
                (i32)std::floor(sprite->offset.y * scaleY),
                (i32)std::ceil(sprite->size.x * scaleX),
                (i32)std::ceil(sprite->size.y * scaleY),
            };
            rect.x = std::max(rect.x, 0);
            rect.y = std::max(rect.y, 0);
            rect.w = std::min(rect.w, std::min(base->w, overlay->w) - rect.x);
            rect.h = std::min(rect.h, std::min(base->h, overlay->h) - rect.y);
            ...
            bool changed = false;
            const u8 *pixels = (const u8 *)overlay->pixels;
            for (i32 y = rect.y; y < rect.y + rect.h && !changed; y++)
            {
                const u8 *row = pixels + (std::size_t)y * overlay->pitch + rect.x * 4;
            ...
            {
                SDL_Rect dst = rect;
                SDL_BlitSurface(overlay, &rect, base, &dst);
            }
        }
...
ZunResult AnmManager::LoadTextureEmbedded(u32 textureIdx, ZunImageInfoEmbedded *imageInfo,
                                          const char *patchName, const AnmRawEntry *rawEntry)
{
    SDL_Surface *surface;
...
#ifdef TH_THPATCH
    ApplyThpatchSprites(converted, patchName, rawEntry);
#endif
...
i32 AnmManager::LoadAnm(i32 textureIdx, AnmRawEntry *rawEntry, i32 spriteIdxOffset, u32 ownsMemory)
{
...
#ifdef TH_THPATCH
            colorPatched = THPatch_WasReplaced(name);
#endif
        }
#ifdef TH_THPATCH
        if (data->mipmapNameOffset != 0 && !colorPatched)
#else
    ...
    {
        // Most thcrap image patches for PCB target textures embedded inside a chained ANM
        // (title character-select art, HUD glyphs, and so on), which the portable loader
        // never opens as files. The name is what thcrap keys them by, so hand it down and
        // let the loader draw the patch over the decoded texture.
        const char *patchName = (const char *)((const u8 *)data + data->nameOffset);
        if (LoadTextureEmbedded(data->textureIdx,
                                (ZunImageInfoEmbedded *)((u8 *)data + data->textureOffset),
                                patchName, data) != ZUN_SUCCESS)
        {
            g_GameErrorContext.Fatal(
