    ...
    if (WAS_PRESSED_RAW(TH_BUTTON_SELECTMENU))
    {
        ...
        for (i = 0; i < 8; i++)
        {
            const char *description =
                THPatch_MusicComment(this->selectedIdx + 1, i,
...
LAB_0043b195:
    ...
    {
        char themeId[16];
        std::snprintf(themeId, sizeof(themeId), "th07_%02d", offset + 1);
        ...
        AnmManager::DrawVmTextFmt(g_AnmManager, arg->titleSprites + offset, 0xc0e0ff, 0x302080,
                                  THPatch_Theme(themeId, arg->trackDescriptors[offset].title));
    ...
    for (offset = 0; offset < 8; offset++)
    {
        ...
        const char *description =
            THPatch_MusicComment(arg->selectedIdx + 1, offset, lineCharBuffer);
        if (*description != '\0')
        {
