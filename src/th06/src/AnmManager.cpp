...
#ifdef TH_THPATCH
    // Patched images are full RGBA PNGs, so the separate alpha file must not overwrite them.
    const char *colorName = (char *)((u8 *)anm + anm->nameOffset);
    const char *colorBase = std::strrchr(colorName, '/');
    bool colorPatched = THPatch_WasReplaced(colorBase != NULL ? colorBase + 1 : colorName);
    if (anm->alphaNameOffset != 0 && !colorPatched)
#else
...
void AnmManager::DrawVmTextFmt(AnmVm *vm, ZunColor textColor, ZunColor shadowColor, const char *fmt, ...)
{
    ...
    va_end(argptr);
#ifdef TH_THPATCH
    TextHelper::SetLayout(vm->sprite->widthPx, TextHelper::ALIGN_LEFT);
#endif
...
#ifdef TH_THPATCH
    TextHelper::SetLayout(0, TextHelper::ALIGN_LEFT);
#endif
...
#ifdef TH_THPATCH
    // The original alignment assumes half-width monospace glyphs; align by the real text
    // extent within the visible sprite instead.
    TextHelper::SetLayout(vm->sprite->widthPx, TextHelper::ALIGN_RIGHT);
...
void AnmManager::DrawStringFormat2(AnmVm *vm, ZunColor textColor, ZunColor shadowColor, const char *fmt, ...)
{
    ...
    // The original alignment assumes half-width monospace glyphs; align by the real text
    // extent within the visible sprite instead.
    TextHelper::SetLayout(vm->sprite->widthPx, TextHelper::ALIGN_CENTER);
    secondPartStartX = vm->sprite->startPixelInclusive.x;
#endif
    this->DrawTextToSprite(vm->sprite->sourceFileIndex, secondPartStartX, vm->sprite->startPixelInclusive.y,
                           vm->sprite->textureWidth, vm->sprite->textureHeight, fontWidth, vm->fontHeight, textColor,
