...
struct Software : GfxInterface
{
    ...
    // Rasterizes all queued draws/clears now (see SoftwareRaster.hpp).
    void Flush();
  ...
  private:
    ...
    f32 *depthBuffer = NULL;

    // Defaults match OpenGL's initial state; these used to be left uninitialized, which only
    // happened to work where fresh heap memory is zeroed.
    i32 viewport[4] = {0, 0, GAME_WINDOW_WIDTH, GAME_WINDOW_HEIGHT}; // x, y, w, h
    ZunColor clearColor = 0;                                          // r, g, b, a
    f32 clearDepth = 1;
    f32 fogNear = 0;
    f32 fogFar = 1;
    ZunColor fogColor = 0;
    f32 depthNear = 0, depthFar = 1;
    bool depthMask = true;
    ...
    ColorOp colorOp = COLOR_OP_MODULATE;
