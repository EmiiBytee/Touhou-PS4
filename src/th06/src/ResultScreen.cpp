...
static const char *CharacterName(i32 idx)
{
#ifdef TH_THPATCH
    static const char *const thpatchIds[4] = {"th06 Stats ReimuA", "th06 Stats ReimuB", "th06 Stats MarisaA",
                                              "th06 Stats MarisaB"};
    if (idx < 4)
    {
        return THPatch_StringDef(thpatchIds[idx], g_CharacterList[idx]);
    }
#endif
    return g_CharacterList[idx];
}
        ...
        if (resultScreen->lastSpellcardSelected != resultScreen->cursor && resultScreen->frameTimer == 20)
        {
            ...
            for (i = resultScreen->lastSpellcardSelected * 10; i < resultScreen->lastSpellcardSelected * 10 + 10; i++)
            {
                ...
                else
                {
                    g_AnmManager->DrawVmTextFmt(&resultScreen->unk_28a0[i % 10], COLOR_RGB(COLOR_WHITE),
                                                #ifdef TH_THPATCH
                                                COLOR_RGB(COLOR_BLACK), THPatch_Spell(i, g_GameManager.catk[i].name));
#else
