...
// BGM title, translated through themes.js when the patch has it. The translation is
// UTF-8, so the note has to be UTF-8 too instead of TH_SONG_NAME's Shift-JIS one.
static void DrawSongName(AnmVm *vm, i32 idx)
{
    const char *path = g_Stage.stdData->songPaths[idx];
    const char *base = std::strrchr(path, '/');
    const char *base2 = std::strrchr(path, '\\');
    base = base2 > base ? base2 : base;
    char id[64];
    std::snprintf(id, sizeof(id), "%s", base != NULL ? base + 1 : path);
    char *dot = std::strrchr(id, '.');
    if (dot != NULL)
    {
        *dot = '\0';
    }

    const char *translated = THPatch_Theme(id, NULL);
    if (translated != NULL)
    {
        g_AnmManager->DrawStringFormat(vm, COLOR_RGB(COLOR_LIGHTCYAN), COLOR_RGB(COLOR_BLACK), "\xE2\x99\xAA%s",
                                       translated);
    }
    else
    {
        g_AnmManager->DrawStringFormat(vm, COLOR_RGB(COLOR_LIGHTCYAN), COLOR_RGB(COLOR_BLACK), TH_SONG_NAME,
                                       g_Stage.stdData->songNames[idx]);
    }
}
#endif
...
#ifdef TH_THPATCH
    this->bombSpellcardBarLength = TextHelper::LastTextWidth() + 16;
#else
...
#ifdef TH_THPATCH
    this->blueSpellcardBarLength = TextHelper::LastTextWidth() + 16.0f;
#else
...
ZunResult Gui::ActualAddedCallback()
{
    ...
    g_AnmManager->DrawStringFormat2(&this->impl->stageNameSprite, COLOR_RGB(COLOR_LIGHTCYAN), COLOR_RGB(COLOR_BLACK),
                                    "%s", THPatch_Stage(g_GameManager.currentStage, 0, g_Stage.stdData->stageName));
#else
    g_AnmManager->DrawStringFormat2(&this->impl->stageNameSprite, COLOR_RGB(COLOR_LIGHTCYAN), COLOR_RGB(COLOR_BLACK),
...
#ifdef TH_THPATCH
    DrawSongName(&this->impl->songNameSprite, 0);
#else
...
#ifdef TH_THPATCH
            DrawSongName(&this->songNameSprite, this->msg.currentInstr->args.music);
#else
...
ZunResult GuiImpl::DrawDialogue() const
{
...
#ifdef TH_THPATCH
    // The patched text.anm widens the dialogue lines to x=64..384, so widen the box with
    // them (thcrap's dialog_box_leftedge / rightedge binhacks).
    f32 boxLeft = g_GameManager.arcadeRegionTopLeftPos.x + 16.0f;
    f32 boxRight = g_GameManager.arcadeRegionTopLeftPos.x + g_GameManager.arcadeRegionSize.x - 16.0f;
#else
    f32 boxLeft = g_GameManager.arcadeRegionTopLeftPos.x + (g_GameManager.arcadeRegionSize.x - 256.0f) / 2.0f - 16.0f;
    f32 boxRight =
        g_GameManager.arcadeRegionTopLeftPos.x + (g_GameManager.arcadeRegionSize.x - 256.0f) / 2.0f + 256.0f + 16.0f;
#endif
    vertices[0].position =
        ZunVec4(boxLeft,
                384.0f, 0.0f, 1.0f);
    ...
    vertices[1].position = ZunVec4(boxRight,
                                   384.0f, 0.0f, 1.0f),
    ...
    //           sizeof(ZunVec3));
        ...
        vertices[3].position = ZunVec4(boxRight,
                                       384.0f + dialogueBoxHeight, 0.0f, 1.0f);
...
void Gui::DrawStageElements() const
{
...
#ifdef TH_THPATCH
        // 128 is half of the original 256px name sprite; the patched ones are 512px wide.
        this->impl->bombSpellcardBackground.pos.x += this->bombSpellcardBarLength * 16.0f / 15.0f / 2.0f -
                                                     this->impl->bombSpellcardName.sprite->widthPx / 2.0f - 16.0f;
#else
    ...
    if (this->impl->enemySpellcardName.flags.isVisible)
    {
        ...
        this->impl->enemySpellcardBackground.pos.x +=
            this->impl->enemySpellcardName.sprite->widthPx / 2.0f - this->blueSpellcardBarLength * 16.0f / 15.0f / 2.0f;
#else
