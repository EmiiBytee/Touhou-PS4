...
static const struct
{
    ...
    // Piglet (the PS4's GL) only works in mini apps, so a big app talks to the GPU through
    // GNM, and keeps the software rasterizer as the fallback if that fails to start.
} s_RenderBackends[] = {
#ifdef TH_PS4_GNM
    {"GNM (PS4 GPU)", GnmBackend::Init},
#endif
    {"Software + SceVideoOut", Software::Init}};
#else
...
#ifdef TH_PS4
    {
        // The pillarbox bars are never drawn to, so wipe the whole framebuffer (both
        // swap buffers) after switching between 4:3 and 16:9.
        static bool s_LastWidescreen;
        static i32 s_ClearFrames = 2;
        if (s_LastWidescreen != g_PS4Widescreen)
        {
            s_LastWidescreen = g_PS4Widescreen;
            s_ClearFrames = 2;
        }
        if (s_ClearFrames > 0)
        {
