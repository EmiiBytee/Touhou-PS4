    ...
    case STATE_KEYCONFIG:
        ...
        if (32 <= menu->stateTimer)
        {
...
#ifdef TH_PS4
            // The pad is the only input device here, so the D-pad has to keep navigating this
            // menu instead of being bound to whatever row the cursor is on. The direction rows
            // (4-7) stay on the D-pad / left stick.
            bool isDpad = sVar1 >= SDL_CONTROLLER_BUTTON_DPAD_UP && sVar1 <= SDL_CONTROLLER_BUTTON_DPAD_RIGHT;
            bool rebindable = !isDpad && (menu->cursor < 4 || menu->cursor == 8);
#else
            bool rebindable = true;
#endif
            if (rebindable && sVar1 < SDL_CONTROLLER_BUTTON_MAX && g_LastJoystickInput != sVar1)
            {
            ...
            if (WAS_PRESSED(TH_BUTTON_SELECTMENU))
            {
                ...
                case 9:
#ifdef TH_PS4
                    // The original defaults are DirectInput button numbers and leave the D-pad
                    // unbound; reset to the SDL defaults instead.
                    memcpy(menu->controlMapping, &g_DefaultControllerMapping, sizeof(ControllerMapping));
                    break;
#endif
...
{
#ifdef TH_PS4
    g_PS4Widescreen = this->windowed != 0;
#endif
    ...
    if (this->stateTimer >= 32)
    {
        ...
        if (WAS_PRESSED(TH_BUTTON_SELECTMENU))
        {
            ...
            case CURSOR_OPTIONS_POS_EXIT:
                ...
                g_SoundPlayer.PlaySoundByIdx(SOUND_BACK);
#ifdef TH_PS4
                // Restarting recreates the window and GL context, which the PS4 can't do.
                // None of these need it here: apply them directly.
