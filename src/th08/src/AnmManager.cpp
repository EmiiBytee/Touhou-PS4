...
#endif
#ifdef TH_PS4
// Right edge, in text-buffer pixels, of the sprite the next DrawTextInner fills: TextOutA
// narrows anything that would run past it instead of letting the sprite cut it off.
// Defined in src/modern/linux/linux_compat.cpp.
extern "C" int g_Ps4TextFitRight;
#endif
#include <stdarg.h>
#include <stdio.h>
#ifdef TH_THPATCH
...
#ifdef TH_THPATCH
// Every menu description, help line and bomb name reaches the screen through DrawTextLeft,
// DrawTextRight or DrawTextCentered, and the translation patch replaces them by their exact
// Shift-JIS text (see THPatch_TranslateText). The format is looked up first, so lines with
// numbers in them still match; the finished line second, for text that arrives through a
// plain "%s". English is longer than the Japanese it replaces, so the callers' buffers are
// larger in this build and the formatting is bounded.
static void FormatTranslatedText(char *buf, size_t size, const char *fmt, va_list args)
{
    vsnprintf(buf, size, THPatch_TranslateText(fmt), args);
    const char *translated = THPatch_TranslateText(buf);
    if (translated != buf)
    {
        strncpy(buf, translated, size - 1);
        buf[size - 1] = '\0';
    }
}
#define TH08_TEXT_BUFFER_SIZE(original) 256
#else
#define TH08_TEXT_BUFFER_SIZE(original) original
#endif

#ifdef TH_PS4
// TextHelper draws at twice the texture's resolution, starting from the sprite's left edge,
// and copies back at most sprite-width * 2 (and 1024) buffer pixels: whatever lies past the
// sprite's right edge or past that copy is lost.
static i32 TextFitRight(AnmVm *vm)
{
    i32 right = (i32)(vm->loadedSprite->endPixelInclusive.x * 2.0f);
    const i32 copied = (i32)(vm->loadedSprite->width * 2.0f);
    if (copied > 0 && copied < right) right = copied;
    return right < 1024 ? right : 1024;
}
#endif
...
namespace modern
{
// src/modern/ps4/th08_thpatch.cpp
...
#ifdef TH_THPATCH
    // Translation patches replace ANM art sprite by sprite, over the texture just built.
    modern::ApplyThpatchSprites(anmLoaded->textures[entryNumber].texture, startOfEntry,
                                (const char *)((u8 *)startOfEntry + startOfEntry->nameOffset));
#endif
...
{
    char buf[TH08_TEXT_BUFFER_SIZE(128)];
    ...
    va_start(args, fmt);
#ifdef TH_THPATCH
    FormatTranslatedText(buf, sizeof(buf), fmt, args);
#else
...
#ifdef TH_PS4
    g_Ps4TextFitRight = TextFitRight(vm);
#endif
...
{
    char buf[TH08_TEXT_BUFFER_SIZE(72)];
    int x;
    int fontWidth = vm->fontWidth <= 0 ? 15 : vm->fontWidth;
...
#ifdef TH_PS4
    g_GameErrorContext.Log("ps4 retry: capture anm=%d entries=%p src=%d,%d %dx%d dst=%d,%d %dx%d\r\n", captureAnmIdx,
                           (void *)this->anmFiles[captureAnmIdx].textures, srcX, srcY, srcW, srcH, dstX, dstY, dstW,
                           dstH);
#endif
#ifdef TH08_MODERN_PORT
    // The entry array itself is dereferenced below without a check; a slot whose ANM is not
    // loaded would take the process down rather than skip the capture.
    if (this->anmFiles[captureAnmIdx].textures == NULL)
    {
        return;
    }
#endif
