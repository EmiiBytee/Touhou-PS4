...
#ifdef TH_THPATCH
// Music Room text from the translation patch. themes.js names tracks by their position in
// the room ("th08_01" is the first entry, whatever its MIDI is called); musiccmt.js has the
// comment prose of each track. Line 0 of a track's block in musiccmt.txt is its "No.N title"
// heading, so the prose starts at line 1.
static const char *TranslatedTrackTitle(i32 track, const char *fallback)
{
    char id[16];
    snprintf(id, sizeof(id), "th08_%02d", track + 1);
    return THPatch_GameText(THPatch_Theme(id, NULL), fallback);
}

static const char *TranslatedTrackLine(i32 track, i32 line, const char *fallback)
{
    if (line == 0)
    {
        static char heading[128];
        const char *title = TranslatedTrackTitle(track, NULL);
        if (title == NULL)
        {
            return fallback;
        }
        snprintf(heading, sizeof(heading), "No.%d %s", track + 1, title);
        return heading;
    }
    // A translated track can have fewer lines than the Japanese; those stay empty.
    if (THPatch_MusicComment(track + 1, 0, NULL) != NULL)
    {
        return THPatch_GameText(THPatch_MusicComment(track + 1, line - 1, NULL), "");
    }
    return fallback;
}
#endif
...
#ifdef TH_THPATCH
            // The "not heard yet" warning is the executable's own text and is translated
            // where it is drawn; a track's own lines come from the patch here.
            const char *line = buf1;
            if (this->selectedSongIndex == this->cursor || this->bgmUnlocked[this->cursor])
            {
                line = TranslatedTrackLine(this->cursor, i, buf1);
            }
            if (line[0] != '\0')
            {
                this->descriptionVms[i].flag1 = true;
                g_AnmManager->DrawTextLeft(&this->descriptionVms[i], 0xffe0c0, 0x300000, "%s", line);
            }
#else
            if (buf1[0] != '\0')
            {
...
#ifdef TH_THPATCH
        g_AnmManager->DrawTextLeft(&this->descriptionVms[7], 0xffe0c0, 0x300000, "%s",
                                   TranslatedTrackLine(this->cursor, 0, buf2));
#else
...
out:
    ...
    for (i = 0; i < musicRoom->numDescriptors; i++)
    {
        ...
        {
#ifdef TH_THPATCH
            g_AnmManager->DrawTextLeft(&musicRoom->songNameVms[i], 0xc0e0ff, 0x302080, "%s",
                                       TranslatedTrackTitle(i, musicRoom->trackDescriptors[i].title));
#else
