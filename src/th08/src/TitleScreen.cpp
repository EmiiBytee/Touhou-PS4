...
#ifdef TH_PS4
                // On Windows a changed Mode or Graphic option restarts the game to create a new
                // Direct3D device. The PS4 device cannot be torn down and rebuilt that way, and
                // it doesn't need to be: Mode is 4:3 / 16:9 here, which the renderer reads every
                // frame, and colour depth only affects textures loaded from now on.
                if (false)
#else
            ...
            for (i2 = 0; i2 < TITLE_SPELL_CARD_SPELLCARDS_PER_PAGE; i2++)
            {
                ...
                if (g_GameManager.HasSpellCardBeenEncountered(spellCardNumber2, SHOT_ALL))
                {
                    g_AnmManager->DrawTextLeft(&this->spellCardNameVms[i2], COLOR_TEXT_WHITE, 0,
                                               TH08_SPELL_DISPLAY_NAME(spellCardNumber2, g_GameManager.catkData[spellCardNumber2].spellName));
                }
                else
                {
    ...
    case TITLE_REPLAY_MENU_SELECT_REPLAY:
        ...
        if (WAS_PRESSED(TH_BUTTON_SHOOT | TH_BUTTON_ENTER))
        {
            ...
            g_AnmManager->DrawTextLeft(&this->spellCardNameVms[0], COLOR_TEXT_WHITE, 0,
                                       TH08_SPELL_DISPLAY_NAME(this->replays[this->selectedReplay].spellcardNumber, this->replays[this->selectedReplay].spellcardName));
