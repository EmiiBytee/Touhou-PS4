...
// A spell practice retry dies somewhere in GameplaySetupThread. One line before each step,
// through the game's own log, so the last one written names the step that did not finish.
#ifdef TH_PS4
#define TH_PS4_SETUP_STEP(name) g_GameErrorContext.Log("ps4 setup: " name "\r\n")
#else
#define TH_PS4_SETUP_STEP(name)
#endif
...
#ifdef TH_PS4
    g_GameErrorContext.Log("ps4 setup: start keep=%d initial=%d release=%d spell=%d card=%d stage=%d diff=%d\r\n",
                           (i32)g_Supervisor.keepStageResources, (i32)g_Supervisor.isInitialStageLoad,
                           (i32)g_Supervisor.releaseResourcesOnRestart, (i32)g_GameManager.flags.isSpellPractice,
                           (i32)g_GameManager.currentSpellCardNumber, (i32)g_GameManager.currentStage,
                           (i32)g_GameManager.difficulty);
#endif
...
i32 InitializeScoreData()
{
    ...
    if (g_GameManager.flags.isPracticeMode)
    {
        ...
        // ResultScreen::WriteScore only serializes the practice records whose flag is set,
        // and nothing in the reconstruction ever sets it, so practice attempts and high
        // scores never reached score.dat. This writes the flag that is one byte past
        // shotNumber, which is the record's own index: ParsePSCR files each record by it,
        // so forcing it to 1 would also collapse every shot type's practice data into slot
        // one on the next load. Not verified against the target with reccmp.
        g_GameManager.pscrData[g_GameManager.shotType].shouldSerialize = 1;
#else
