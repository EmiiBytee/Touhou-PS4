...
namespace
{
// GameErrorContext lines carry the original CRLF; strip it so each one is a single line in
// ps4_log.txt, and keep the copy bounded because the game logs whole paragraphs at a time.
// Capped: every line is flushed to disk and to the kernel log, so a routine that logs per
// frame would cost real frame time. The cap is far above what startup and the menus
// produce, which is the part worth having when something dies.
int g_PS4LoggedLines;
const int kPS4MaxLoggedLines = 4000;

void PS4_LogTrimmed(const char *text)
{
    if (g_PS4LoggedLines >= kPS4MaxLoggedLines)
    {
        if (g_PS4LoggedLines == kPS4MaxLoggedLines)
        {
            g_PS4LoggedLines++;
            PS4_Log("game: (log cap reached, further game log lines are dropped)");
        }
        return;
    }
    g_PS4LoggedLines++;
...
#ifdef TH_THPATCH
    // Archive entries the translation patch ships whole (base_tsa's text.anm, with its
    // wider dialogue rows) stand in for the game's own. Files outside the archive -- saves,
    // replays -- never do.
    if (!isExternalResource)
    {
        u32 patchedSize = 0;
        u8 *patched = THPatch_LoadReplacement(path, &patchedSize);
        if (patched != NULL)
        {
            if (fileSize != NULL)
            {
                *fileSize = (i32)patchedSize;
            }
            return patched;
        }
    }
...
#if defined(TH08_MODERN_PORT) && !defined(TH_PS4)
            // Development aid for the Linux build: with TH08_DUMP_DIR set, every archive entry
            // is written out exactly as the game receives it (decompressed and decrypted), which
            // is how ANM sheets and MSG scripts are inspected for the PS4 port.
            {
                const char *dumpDir = getenv("TH08_DUMP_DIR");
                if (dumpDir != NULL && data != NULL)
                {
                    char dumpPath[512];
                    snprintf(dumpPath, sizeof(dumpPath), "%s/%s", dumpDir, entryname);
                    FILE *dump = fopen(dumpPath, "wb");
                    if (dump != NULL)
                    {
                        fwrite(data, 1, fileSize != NULL ? (size_t)*fileSize : (size_t)size, dump);
                        fclose(dump);
                    }
                }
            }
#endif
#ifdef TH_THPATCH
            // Dialogue scripts get their translated lines once decoded (thpatch's msg08 path).
            {
                u32 patchedSize = fileSize != NULL ? (u32)*fileSize : (u32)size;
                data = THPatch_Transform(entryname, data, &patchedSize);
                if (fileSize != NULL)
                {
                    *fileSize = (i32)patchedSize;
                }
...
#ifdef TH_PS4
    // The game's own log is normally only written out by GameErrorContext::Flush() at exit,
    // which a crash never reaches. On PS4 every line also goes to ps4_log.txt and the kernel
    // log as it happens, so the last thing the game reported is always readable afterwards.
    PS4_LogTrimmed(tmpBuffer);
#endif
