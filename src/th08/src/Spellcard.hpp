...
#ifdef TH_THPATCH
#include "thpatch.hpp"
// The name a spell card is shown with. The Japanese one stays what the game stores (score.dat,
// replays); the translation patch's spells.js supplies the English one by spell number.
#define TH08_SPELL_DISPLAY_NAME(number, name) THPatch_SpellText((number), (name))
#else
#define TH08_SPELL_DISPLAY_NAME(number, name) (name)
#endif
