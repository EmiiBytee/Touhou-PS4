...
namespace
{
// ZUN's unlock code cannot be entered on a DualShock 4: it wants Shift or Ctrl held while
// Right, Left, D and Q are typed, and the pad has none of those keys. The PS4 build reads
// this sequence instead: Up Up Down Down Left Right Left Right Cross Circle.
const u16 g_Ps4CheatSequence[] = {
    TH_BUTTON_UP,    TH_BUTTON_UP,   TH_BUTTON_DOWN,  TH_BUTTON_DOWN,  TH_BUTTON_LEFT,
    TH_BUTTON_RIGHT, TH_BUTTON_LEFT, TH_BUTTON_RIGHT, TH_BUTTON_SHOOT, TH_BUTTON_BOMB,
};

// Everything the sequence watches. A watched press that is not the step it expects starts
// it over, so the code has to be entered cleanly.
const u16 PS4_CHEAT_WATCHED =
    TH_BUTTON_DIRECTION | TH_BUTTON_SHOOT | TH_BUTTON_BOMB | TH_BUTTON_MENU | TH_BUTTON_ENTER;

// What ZUN's code grants: every difficulty cleared for every shot type, and one recorded
// attempt at every spell card so the spell card results list is fully populated.
void Ps4UnlockEverything()
{
    i32 i;
    i32 j;
    ...
    for (i = 0; i < SHOT_ALL + 1; i++)
    {
        ...
        {
            g_GameManager.clrdData[i].difficultiesClearedWithoutRetries[j] |= 0xffff;
            g_GameManager.clrdData[i].difficultiesClearedWithRetries[j] |= 0xffff;
        }
    }
    ...
    for (i = 0; i < SPELLCARD_COUNT_SPELLCARDS; i++)
    {
        ...
        {
            g_GameManager.catkData[i].inGameHistory.attempts[j]++;
        }
    }
...
// Feeds this frame's press to the sequence. Returns true when the press belongs to the code
// and the caller must not also act on it: Cross and Circle are the last two steps and would
// otherwise leave this screen before the code could finish.
ZunBool Ps4FeedCheatSequence(i32 *step)
{
    const i32 length = (i32)(sizeof(g_Ps4CheatSequence) / sizeof(g_Ps4CheatSequence[0]));
    u16 pressed;
    u16 expected;

    pressed = (u16)(g_CurFrameInput & ~g_LastFrameInput & PS4_CHEAT_WATCHED);
    if (pressed == 0)
    {
        return false;
    }
    ...
    expected = g_Ps4CheatSequence[*step];
    if (pressed != expected)
    {
        // Anything else restarts the sequence -- at one when that press is itself the
        // opening Up, so a false start does not cost the player the whole code.
        *step = pressed == g_Ps4CheatSequence[0] ? 1 : 0;
        return false;
    }
    ...
    // The directions still move the cursor, which is the feedback that the pad is being
    // read at all; only the two face buttons have to be swallowed.
    return expected == TH_BUTTON_SHOOT || expected == TH_BUTTON_BOMB;
}
} // namespace
#endif
...
#ifdef TH_PS4
        // Before the menu sees the press: the code ends on Cross and Circle, which are
        // select and back here.
        if (Ps4FeedCheatSequence(&this->cheatCodeStep))
        {
            break;
        }
#endif
...
i32 ResultScreen::HandleSpellCardScreen()
{
        ...
        this->frameTimer == 10)
    {
        ...
        for (i = this->spellcardPage * 10; i < (this->spellcardPage * 10) + 10; i++)
        {
            ...
            {
                g_AnmManager->DrawTextLeft(&this->textVms[i % 10], COLOR_TEXT_WHITE, 0,
                                           TH08_SPELL_DISPLAY_NAME(spellCardNumber, g_GameManager.catkData[spellCardNumber].spellName));
            }
