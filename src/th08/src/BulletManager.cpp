...
void __fastcall fsincos(f32 *sine, f32 *cosine, f32 angle)
{
    ...
    // Stands in for the x87 fsincos, which produces both results at once. Left empty, it
    // wrote neither, so every laser read an uninitialised sine and cosine: the beams landed
    // at absurd coordinates, the viewport cull in AnmManager::DrawInner dropped them, and
    // their hitboxes were computed from the same garbage.
    *sine = sinf(angle);
    *cosine = cosf(angle);
#else
    // The target emits the instruction inline here; the matching build is left as it was.
    (void)sine;
    (void)cosine;
    (void)angle;
#endif
...
#ifdef TH08_MODERN_PORT
        // The fade ramps in OnTick write "alpha << 24" into the body's colour, so its RGB is
        // black by the time it is drawn, and the original restores the white right here. The
        // reconstruction puts that restore in Laser::color -- a 16-bit sprite index, where
        // the expression truncates to -1 and never reaches the draw -- so every laser that
        // fades in (flags & 1) is drawn black, which under additive blending is invisible.
        // TH07 carries the same line; this is not an exact-match claim, reccmp has not run.
        laser->bodyVm.color1.d3dColor = (laser->bodyVm.color1.d3dColor & 0xff000000) | 0xffffff;
#else
