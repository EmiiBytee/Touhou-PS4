// Process entry point and the target data tables the reconstruction reads at fixed
// addresses. Shared by the Linux and PS4 targets; TH_PS4 selects the pieces that differ
// (crash reporting and how the game data directory is located).
...
#else
// No handler on the PS4: the kernel already writes the faulting thread's registers, rip
// and module map to its log (GoldHEN klog, port 3232) when the process dies, which is the
// report a handler would try to produce.
//
// A handler here also did real harm. musl's <signal.h> describes Linux's struct sigaction,
// but sigaction() itself is the console's FreeBSD one, which reads sa_flags where musl
// keeps sa_mask. It never saw SA_SIGINFO and called the handler with the old BSD
// arguments, so the handler took the signal code for a siginfo pointer and faulted reading
// si_addr (address 0x19). Every crash was then reported as that one, from inside the
// handler, and the fault that started it was lost.
void InstallSignalHandler(int)
{
}
#endif
...
#ifdef TH_PS4
// The PS4 sandbox ignores chdir(), so the shared platform glue rebases every relative
// path instead: either onto /data/touhou/th08 or onto the pkg's own /app0/assets. All
// this has to answer is whether the retail archive is reachable at all.
bool ConfigureDataDirectory()
{
    FILE *archive = fopen("th08.dat", "rb");
    if (archive == NULL)
    {
        PS4_Log("th08.dat not found in " TH_PS4_GAME_DIR " or /app0/assets");
        PS4_Notify("Touhou 8: th08.dat not found in " TH_PS4_GAME_DIR);
        return false;
    }
    fclose(archive);
    unlink(TH_PS4_GAME_DIR "/modern-files.txt");
    unlink(TH_PS4_GAME_DIR "/modern-crash.txt");
    return true;
}
#else
bool ConfigureDataDirectory()
...
{
#ifdef TH_PS4
    // The shared PS4 glue already logs every resolved fopen(); reopening a log file for
    // each archive request on top of that would cost real frame time.
    (void)path;
    return;
#else
...
#ifdef TH_PS4
    PS4_Log("TH08 native engine starting");
    const int result = WinMain(NULL, NULL, NULL, 0);
    PS4_Log("TH08 exiting with %d", result);
    return result;
#else
