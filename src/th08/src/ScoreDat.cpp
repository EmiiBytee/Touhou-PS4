...
ScoreDat *ScoreDat::OpenScore(const char *filename)
{
    ...
    recreate_score_file:
        ...
        // ZunMemory::Alloc is plain malloc, so everything but the three fields assigned
        // below would stay indeterminate on a first run with no score.dat yet. The original
        // got away with it on a freshly zeroed Windows heap; a recycled heap hands the score
        // screen garbage for the version, the checksum and the compressed sizes.
        memset(scoreDat, 0, sizeof(ScoreDat));
#endif
