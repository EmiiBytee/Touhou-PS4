...
void EffectManager::ResetEffects()
{
    ...
    // 0x8B05C is the whole manager in the 32-bit target (see the C_ASSERT beside it), but
    // only its first 0x8B05C of 606376 bytes in the native layout, where Effect grows from
    // 0x360 to 920 bytes. Effects 619-653, the draw-group sentinels and drawGroupTails then
    // survived a spell practice retry: OnDraw walked those stale lists into a radial trail
    // whose vertices ReleaseEffectResources had already freed, and wrote through NULL.
